...
import { discoverDomainsForNiche, isAhrefsEnabled } from '@/lib/ahrefs';
import { checkDisqualifiers } from '@/lib/scoring';
import { CREATOR_DISCOVERY_NICHES } from '@/lib/rei-grove-content';

// POST /api/discovery/search — Body: { nicheKey }
// Runs the niche's keywords through Ahrefs SERP Overview to find real,
// currently-ranking sites (no LLM guessing), dedupes against existing
// prospects, and inserts the new ones as "new" prospects tagged with that
// niche, grouped into one batch (visible under History) — same pattern as
// the n8n bulk-import webhook.
    ...
    try {
      ...
      await sql`
        insert into prospects (
          ...
          'creator', ${c.name}, ${c.website}, 'blog', ${niche.key}, ${c.contentPresence},
